...
class HomeController extends Controller
{
    ...
    public function cambiotasas(Request $request)
    {
        $codigosProductos = [];
        $newtasa          = $request->newtasa;
        $oldtasa          = $request->oldtasa;
        ...
        $productos   = Saprod::where('comercial',$comercialId)->get();

        DB::transaction(function () use ($productos, $oldtasa, $newtasa, &$codigosProductos) {
            foreach ($productos as $producto) {
                $auxcostod  = number_format($producto->costod*$oldtasa ,0,'.','');
                $auxcostod2 = number_format($producto->costod2*$oldtasa,0,'.','');
                $auxcostod3 = number_format($producto->costod3*$oldtasa,0,'.','');

                $producto->costod  = $auxcostod  / $newtasa;
                $producto->costod2 = $auxcostod2 / $newtasa;
                $producto->costod3 = $auxcostod3 / $newtasa;

                $producto->save();
                $codigosProductos[] = $producto->codprod;
            }
        });

        if (!empty($codigosProductos)) {
            Saprodsucursal::whereIn('codprod', $codigosProductos)
        ...
        $comercial->tasapeso = $newtasa;
        $comercial->save();

        return redirect()->back()->with('tasascambiadas',1);
    }
    ...
    {
      // dd(bcrypt('Tiendaskarla25'));
        $tasascambiadas = session('tasascambiadas');

        Session::put('lang', 'sp');
        Session::save();

        if(Auth::user() and auth()->user()->type == 'admin') {
            return view('index',compact(     'tasascambiadas'));
        }else{
